...
def is_valid_pwd(pwd): return len(pwd)>=6 and re.search(r"[A-Za-z]", pwd) and re.search(r"[0-9]", pwd)
def extract_code(t):
    m = re.search(r"\+(\d+)", t)
...
.pub-zone{{position:fixed;bottom:0;left:0;right:0;background:#000;color:white;text-align:center;padding:12px;z-index:9999;font-weight:bold}}
...
.card-pub{{background:#fff3e0;border:2px dashed #ff9800;padding:15px;border-radius:12px;margin-top:15px}}
...
if st.session_state.page=="auth":
    ...
    st.caption("1️⃣ Créer compte → 2️⃣ Se connecter → 3️⃣ Oubli = Mot de passe oublié")
    ...
    with tab1:
        ...
        if st.button("Se connecter", type="primary", use_container_width=True):
            ...
            if u and u.get('pwd')==pwd:
                st.session_state.user=email; st.session_state.page="scanner"; st.rerun()
            else: st.error("Email ou mot de passe incorrect")
    with tab2:
        nom = st.text_input("Nom utilisateur", key="nom_insc").strip()
        c1,c2 = st.columns([2,3])
        with c1: pays = st.selectbox("Choix du pays", ["🇨🇮 +225 (CI)","🇸🇳 +221 (SN)","🇲🇱 +223 (ML)","🇬🇳 +224 (GN)","🇧🇫 +226 (BF)","🇧🇯 +229 (BJ)","🇹🇬 +228 (TG)","🇳🇪 +227 (NE)","🇨🇲 +237 (CM)","🇫🇷 +33 (FR)"])
        with c2: numero = st.text_input("Numéro", placeholder="0771845766").strip()
        email_r = st.text_input("Email", key="email_insc").strip()
        ...
        if st.button("Créer mon compte", type="primary", use_container_width=True):
            if not nom or not numero or not email_r or not pwd1 or not pwd2: st.error("Remplis tous les champs")
            elif not is_valid_pwd(pwd1): st.error("Mot de passe doit avoir lettres + chiffres, ex: baba2000")
            elif pwd1!=pwd2: st.error("Mots de passe différents")
            elif email_r in users: st.error("Email déjà utilisé, va dans Connexion")
            else:
                users[email_r]={'nom':nom,'wave':f"{extract_code(pays)} {numero}",'pays':pays,'pwd':pwd1,'scans':0,'is_vip':False,'history':[],'bonus_scans':0}
                save_json(USERS_FILE, users); st.success("Compte créé! Va dans Connexion"); st.balloons()
    with tab3:
        email_f = st.text_input("Email de ton compte", key="email_forgot").strip()
        if st.button("Envoyer code"):
            if email_f in users:
                code=str(random.randint(100000,999999)); st.session_state.reset_code=code; st.session_state.reset_email=email_f; st.success(f"Code démo: {code}")
            else: st.error("Email non trouvé")
        if st.session_state.reset_code:
            c_in = st.text_input("Code reçu").strip()
            npwd = st.text_input("Nouveau mot de passe", type="password", key="newpwd2")
            if st.button("Réinitialiser"):
                if c_in==st.session_state.reset_code:
                    if not is_valid_pwd(npwd): st.error("Doit avoir lettres + chiffres")
                    else: users[st.session_state.reset_email]['pwd']=npwd; save_json(USERS_FILE, users); st.success("Changé! Va dans Connexion"); st.session_state.reset_code=None
...
with st.sidebar:
    try: st.image("logo.jpeg", width=80)
    except: pass
    ...
    if st.button("Déconnexion"): st.session_state.user=None; st.session_state.page="auth"; st.rerun()
...
if menu=="📸 Scanner":
    ...
    if not user['is_vip'] and scans_used>=5:
        ...
        st.markdown(f"""<div class="card-vip"><h3 style="color:gold;">👑 Deviens VIP - 1500F</h3><p>✅ Illimité à vie<br>✅ Sans pub<br><b>Paiement sécurisé Wave</b></p></div>""", unsafe_allow_html=True)
        ...
        if st.button("✅ J'ai payé - Activer mon VIP", use_container_width=True):
            users[user_email]['is_vip']=True; save_json(USERS_FILE, users); st.balloons(); st.success("VIP activé!"); st.rerun()
        st.markdown("---")
        st.markdown("""<div class="card-pub"><h4 style="color:#ff9800;margin:0;">🎁 Option Gratuite - DIFFERENT DU VIP</h4><p>Regarde une pub pour gagner 1 scan</p></div>""", unsafe_allow_html=True)
        if st.button("▶️ Regarder une pub (30s) pour 1 scan gratuit", use_container_width=True):
            users[user_email]['bonus_scans']=users[user_email].get('bonus_scans',0)+1; save_json(USERS_FILE, users); st.success("+1 scan"); st.rerun()
        st.stop()
    ...
    if uploaded:
        ...
        if st.button("Analyser", type="primary", use_container_width=True):
            ...
            detail="Aucun haram" if "HALAL" in result else "Gélatine porcine / Alcool suspect"
...
elif menu=="🎮 Zone de Jeux":
    st.title("🎮 Zone de Jeux")
    st.info("Quiz: 18/20 = 2 scans offerts")
    q1=st.radio("Le porc est Halal?",["Non","Oui"],key="q1")
    q2=st.radio("La gélatine porcine est?",["Haram","Halal"],key="q2")
    ...
    st.write(f"Wave: {user.get('wave')}")
    st.write(f"Email: {user_email}")
    st.write(f"VIP: {user.get('is_vip')}")

elif menu=="⚙️ Paramètres":
    st.title("⚙️ Paramètres")
    nn=st.text_input("Changer nom", value=user.get('nom',''))
    if st.button("Sauvegarder"): users[user_email]['nom']=nn; save_json(USERS_FILE, users); st.success("Sauvé")

elif menu=="🍎 Aliments":
    st.title("🍎 Aliments")
    st.markdown("<div class='card'>✅ Halal: fruits, légumes, boeuf/poulet halal<br>❌ Haram: porc, alcool<br>⚠️ Douteux: E471, gélatine</div>", unsafe_allow_html=True)

elif menu=="📋 Ma Liste":
    st.title("📋 Ma Liste")
    for h in reversed(user.get('history',[])):
        st.markdown(f"<div class='card'>{h['date']} - {h['result']}</div>", unsafe_allow_html=True)
    ...
    st.title(menu)
    st.markdown(f"<div class='card'>Contenu de {menu} bientôt disponible</div>", unsafe_allow_html=True)
    if menu=="📖 Notice": st.write("1.Crée compte 2.Connecte-toi 3.Scanne 5 fois 4.Deviens VIP ou regarde pub")
    if menu=="🌐 Langue": st.selectbox("Langue", ["Français","English","العربية"])
    if menu=="🎧 Coran - Imam Matroud": st.audio("https://www.soundhelix.com/examples/mp3/SoundHelix-Song-1.mp3")
    if menu=="📜 Hadiths": st.write("Les actions ne valent que par les intentions - Bukhari")
    if menu=="🤲 Douas du Jour": st.write("Bismillah avant manger, Alhamdoulillah après")
